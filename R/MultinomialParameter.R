...
setMethod(
  ...
  function(object) {
    if (length(object@proportions) > 0) {
      ...
      for (i in seq_along(object@proportions)) {
        cat("*** Cluster", i, "\n")
        ...
        if (length(object@scatter) > 1) {
          if (length(dim(object@scatter)) == 2) {
            ...
            if (nrow(object@scatter[[i]]) > 1) {
              cat("* scatter    = |", formatC(object@scatter[[i]][1, 1:object@factor[1]],
                digits = 4, width = 10,
                format = "f"
              ), "|\n")
              for (j in 2:nrow(object@scatter[[i]])) {
                cat("               |", formatC(object@scatter[[i]][j, 1:object@factor[j]],
                  digits = 4, width = 10,
                  format = "f"
                ), "|\n")
              }
...
setMethod(
  ...
  function(object, ...) {
    if (length(object@proportions) > 0) {
      for (k in seq_along(object@proportions)) {
        cat("*                  Cluster ", k, ": \n")
        ...
        if (length(object@scatter) > 1) {
          if (length(dim(object@scatter)) == 2) {
            ...
            if (nrow(object@scatter[[k]]) > 1) {
              cat("                            Scatter = |", formatC(object@scatter[[k]][1, 1:object@factor[1]],
                digits = 4,
                width = 10, format = "f"
              ), "|\n")
              for (j in 2:nrow(object@scatter[[k]])) {
                cat("                                      |", formatC(object@scatter[[k]][j, 1:object@factor[j]],
                  digits = 4,
                  width = 10, format = "f"
                ), "|\n")
              }
