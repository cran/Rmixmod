...
CompositeData::CompositeData(Data *bdata, Data *gdata) : Data(bdata->_nbSample, bdata->_pbDimension + gdata->_pbDimension)
{
...
void CompositeData::input(const DataDescription &dataDescription)
{
	...
	// for (std::vector<ColumnDescription*>::const_iterator it=dataDescription.getAllColumnDescription().begin(); it !=
	// dataDescription.getAllColumnDescription().end(); it++) {
	...
	_dataComponent.resize(2);
	// bData
	DataDescription bDataDescription(dataDescription.getNbSample(), dataDescription.getNbColumn(), bColumnDescription,
	                                 dataDescription.getFormat(), dataDescription.getFileName(),
	                                 dataDescription.getInfoName());
	_dataComponent[0] = bDataDescription.getData();
	_dataComponent[0]->input(bDataDescription);
	bDataDescription.releaseData();
	// gdata
	DataDescription gDataDescription(dataDescription.getNbSample(), dataDescription.getNbColumn(), gColumnDescription,
	                                 dataDescription.getFormat(), dataDescription.getFileName(),
	                                 dataDescription.getInfoName());
