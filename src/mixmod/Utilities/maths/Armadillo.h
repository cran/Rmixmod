...
public:
	DiagonalMatrix(int dim) { _value = new arma::Col<double>(dim); }

	~DiagonalMatrix() { delete _value; }

	double *Store() { return _value->memptr(); }

	double &operator[](int i) { return (*_value)[i]; }

	int Nrow() { return _value->n_rows; }

	arma::Col<double> *getValue() { return _value; }
	...
	bool isAlmostEqual(DiagonalMatrix &dm, double epsilon) const
	{
		...
		for (int i = 0; i < dm.Nrow(); i++) {
			if (fabs(_value->memptr()[i] - dm.getValue()->memptr()[i]) > epsilon)
				res = false;
		}
		return res;
	}
...
public:
	Matrix(int nrow, int ncol) { _value = new arma::Mat<double>(nrow, ncol); }

	~Matrix() { delete _value; }

	double *Store() { return _value->memptr(); }

	double &operator()(int i, int j) { return (*_value)(i, j); }
	...
	int Ncol() { return _value->n_cols; }

	arma::Mat<double> *getValue() { return _value; }
	...
	bool isAlmostEqual(Matrix &m, double epsilon) const
	{
		...
		for (int j = 0; j < ncol; j++) {
			for (int i = 0; i < ncol; i++) {
				if (fabs(fabs((_value)->memptr()[i * (_value->n_cols) + j]) -
				         fabs(m.getValue()->memptr()[i * (_value->n_cols) + j])) > epsilon)
					res = false;
			}
		}
		return res;
...
public:
	// nrow == ncol
	SymmetricMatrix(int nrow) { _value = new arma::Mat<double>(nrow, nrow); }
	...
	bool isAlmostEqual(SymmetricMatrix &sm, double epsilon) const
	{
		...
		for (int j = 0; j < ncol; j++) {
			for (int i = 0; i < j + 1; i++) {
				if (fabs((_value)->memptr()[i * (_value->n_cols) + j] - sm.getValue()->memptr()[i * (_value->n_cols) + j]) >
				    epsilon)
					res = false;
			}
		}
