...
DomParameter::DomParameter(string &sFilename) : xmlpp::Document() { set_internal_subset(sFilename, "", ""); }
...
DomParameter::DomParameter(xmlpp::Element *root) { _root = create_root_node_by_import(root); }
...
ParameterDescription *DomParameter::readParameter(string sFilename)
{
	...
	if (_root->get_name() != "Parameter")
		return 0;

	//------------------------
	// Declaration of variables
	//------------------------
	xmlpp::Element *elementName, *elementNbVariable, *elementNbCluster, *elementFormat, *elementParameterFilename,
	    *elementModel, *elementListNbFactor;
	...
	if (isBinary(modelName)) {
		...
		return new ParameterDescription(nbCluster, nbVariable, Global::vNbFactor, format, parameterFilename, sName, modelName);
	} else if (isHeterogeneous(modelName)) {
		return new ParameterDescription(nbCluster, Global::nbVariables_binary, Global::nbVariables_gaussian, Global::vNbFactor,
		                                format, parameterFilename, sName, modelName);
	} else { // gaussian
		// if(!isGeneral(modelName)) modelName = Gaussian_pk_Lk_Ck; // because ParameterDescription creates a
		// GaussianGeneralParameter object anyway
		return new ParameterDescription(nbCluster, nbVariable, format, parameterFilename, sName, modelName);
	}
	//}//
}
