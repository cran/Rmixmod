...
DataDescription * DomData::readDataFile(const string & sFilename)
{
  ...
  LabelDescription * DomData::readLabelsFromFile(const string & sFilename, int64_t discriminantVar) {
	// Data * res = NULL;//return the new Data for NvInput from .mxd file
    xmlpp::DomParser parser;
    parser.parse_file(sFilename);
    xmlpp::Document *doc = parser.get_document();
    ...
    if(listColumnNode){
      ...
      for (auto it=children.begin(); it != children.end(); ++it){
        ...
        int64_t index = std::stoll(n->get_attribute_value("Num")) - 1;
        string nameColumn = n->get_attribute_value("Name");
        if(index == discriminantVar-1){
          if(columnType != IOStreamColumnType::Qualitative) throw IOStreamErrorType::badElementInDataXML; //TO DO: a better messg
          int64_t nbFactor = std::stoll(n->get_attribute_value("NbFactor"));
          //creation of Column
          auto column = new QualitativeColumnDescription(index, nbFactor);
          column->setName(nameColumn);
          ...
          auto column = new UnusedColumnDescription(index);
          column->setName(nameColumn);
          vColumnDescription.push_back(column);
        }
	...
	//initialization of DataDescription
	DataDescription ddesc(iNbSample, iNbColumn, 
                               vColumnDescription, iFormat, iDataFilename, iName);
    Data *data = ddesc.getData();
    std::vector<int64_t> labels(data->getNbSample());
    for(int i = 0; i< data->getNbSample(); i++){
      const BinarySample* sample = dynamic_cast<const BinarySample*>(data->getDataI(i));
      labels[i] = sample->getDataValue(0);
    }
    return new XEM::LabelDescription( labels.size(), labels );
}
