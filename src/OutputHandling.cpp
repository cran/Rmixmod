...
OutputHandling::OutputHandling(XEM::ModelOutput *MOutput, Rcpp::S4 &xem, const XEM::DataType dataType)
    : MOutput_(MOutput), xem_(xem), nbCluster_((int)MOutput_->getNbCluster())
{
...
OutputHandling::OutputHandling(XEM::Parameter *par, Rcpp::S4 &xem, const XEM::DataType dataType, int nbCluster)
    : MOutput_(nullptr), xem_(xem), nbCluster_(nbCluster)
{
...
void OutputHandling::setGaussianParameter(XEM::GaussianEDDAParameter const *parArg)
{
	...
	const XEM::GaussianEDDAParameter *gParam =
	    parArg != nullptr
	        ? parArg
	        : dynamic_cast<XEM::GaussianEDDAParameter const *>(MOutput_->getParameterDescription()->getParameter());
	...
	for (int i = 0; i < nbCluster_; i++) {
		...
		std::unique_ptr<double *[], XEM::TabDeleter<double>> stoa(
		    matrixVariance[i]->storeToArray(), XEM::TabDeleter<double>(matrixVariance[i]->getPbDimension()));
...
void OutputHandling::setMultinomialParameter(XEM::BinaryParameter const *parArg)
{
	...
	const XEM::BinaryParameter *bParam =
	    parArg != nullptr ? parArg
	                      : dynamic_cast<XEM::BinaryParameter const *>(MOutput_->getParameterDescription()->getParameter());
...
{
	XEM::CompositeParameter *cParam =
	    parArg != nullptr ? parArg
	                      : dynamic_cast<XEM::CompositeParameter *>(MOutput_->getParameterDescription()->getParameter());
	// get pointer to gaussian parameters
	const XEM::GaussianEDDAParameter *gParam =
	    dynamic_cast<XEM::GaussianEDDAParameter const *>(cParam->getGaussianParameter());
	// get pointer to multinomial parameters
	...
	for (int i = 0; i < nbCluster_; i++) {
		varianceMatrices[i] =
		    Conversion::CMatrixToRcppMatrix(nbVariable_Gaussian, nbVariable_Gaussian, matrixVariance[i]->storeToArray());
	}
	// add variances to S4 object
