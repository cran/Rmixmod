...
RcppExport SEXP clusteringMain(SEXP xem)
{
	...
	if (XmlOut.size() > 0)
		xmlOutput = Rcpp::as<std::vector<std::string>>(XmlOut)[0];
	...
	if (xmlInput == "") {
		...
		if (numFormat == "hexBinary")
			iomode = XEM::IoMode::BINARY;
		...
		for (int i = 0; i < cInput->getNbClusterSize(); i++)
			vectorNbCluster(i) = cInput->getNbCluster(i);
		mixmodClustering.slot("nbCluster") = vectorNbCluster;
		Rcpp::StringVector vectorCrit(cInput->getNbCriterion());
		for (int i = 0; i < cInput->getNbCriterion(); i++)
			vectorCrit(i) = CriterionNameToString(cInput->getCriterionName(i));
		...
		for (int i = 0; i < cInput->getStrategy()->getNbAlgo(); i++)
			vectorAlgo(i) = AlgoNameToString(cInput->getStrategy()->getAlgo(i)->getAlgoName());
		strategy.slot("algo") = vectorAlgo;
		strategy.slot("nbTry") = Rcpp::NumericVector(1, cInput->getStrategy()->getNbTry());
		strategy.slot("initMethod") =
		    Rcpp::StringVector(StrategyInitNameToStringApp(cInput->getStrategy()->getStrategyInit()->getStrategyInitName()));
	...
	if (cOutput->atLeastOneEstimationNoError()) {
		...
		for (int i = 0; i < cOutput->getNbClusteringModelOutput(); i++) {
			...
			// ClusteringOutputHandling(cOutput->getClusteringModelOutput(i), iResults,cMain->getInput()->getDataType(),
			// Rcriterion);
			ClusteringOutputHandling(cOutput->getClusteringModelOutput(i), iResults, cMain->getInput()->getDataType(),
			                         cMain->getInput()->getCriterionName());
		...
		// ClusteringOutputHandling(cOutput->getClusteringModelOutput().front(), iResults,cMain->getInput()->getDataType() ,
		// Rcriterion);
		ClusteringOutputHandling(cOutput->getClusteringModelOutput().front(), iResults, cMain->getInput()->getDataType(),
		                         cMain->getInput()->getCriterionName());
