/***************************************************************************
							 SRC/MIXMOD/Kernel/IO/XEMParameterDescription.cpp  description
		...
		ModelName& modelName)
{
	...
	GaussianGeneralParameter* gaussian_parameter =
		new GaussianGeneralParameter(nbCluster, nbVariable_gaussian, _modelType, filename);
	BinaryEkjhParameter* binary_parameter =
		new BinaryEkjhParameter(
			nbCluster, nbVariable_binary, _modelType, tabNbFactor, filename);

	_parameter = new CompositeParameter(binary_parameter, gaussian_parameter, _modelType);
}
